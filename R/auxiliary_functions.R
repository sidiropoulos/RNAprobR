...
#'         window_size), na.rm = TRUE)[(window_side + 1):(length(input_vector) +
#'         window_side)])
#'   }
#' 
...
{
    window_side <- window_size/2-0.5
    return(colMeans(construct_smoothing_matrix(input_vector, window_size), 
...
#' 
#' ## The function is currently defined as
#' function (input_vector, winsor_level = 0.9, only_top = FALSE)
#' {
...
{ 
    ...
    else{input_vector[1:length(input_vector)] <- NA}
...
swinsor_vector <- function(input_vector, window_size, winsor_level=0.9, 
    only_top= FALSE)
{
    #Define function for aligning matrix generated by 
    #construct_smoothing_matrix
    align_smoothing_matrix <- function(input_matrix)
    {
...
GR2norm_df <- function(norm_GR, RNAid="all", norm_methods="all")
{
    ...
    # Parsing normalized file based on the selected RNAid(s) and 
